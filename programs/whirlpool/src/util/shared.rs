...
pub fn validate_price_deviation(
    ...
    let max_price_deviation = whirlpool.protocol_fee_rate as u128; // Example: using protocol_fee_rate as a deviation factor
    let lower_bound = price_limit.saturating_sub(max_price_deviation);
    let upper_bound = price_limit.saturating_add(max_price_deviation);

    if current_price < lower_bound || current_price > upper_bound {
        return Err(ErrorCode::PriceDeviationExceeded.into());
    }
    Ok(())
}
