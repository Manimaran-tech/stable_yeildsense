...
pub fn handler<'info>(
    ...
    // --- SECURITY: Price Oracle Validation ---
    // If oracle account is initialized, validate that the pool price is not too far from expected
    if let Ok(Some(adaptive_fee_info)) = oracle_accessor.get_adaptive_fee_info() {
        validate_price_deviation(
            whirlpool,
            whirlpool.sqrt_price,
            sqrt_price_limit, // Using sqrt_price_limit as a proxy for the expected "safe" price in this context
        )?;
    }
