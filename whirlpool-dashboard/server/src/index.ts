...
import { standardRateLimiter, sensitiveActionLimiter } from "./middleware/rateLimiter.js";
import { verifyWebhookSignature } from "./middleware/webhookSecurity.js";
import { getChallenge, login } from "./handlers/auth.js";
...
const metricsMiddleware = promBundle({
    includeMethod: true,
    includePath: true,
    customLabels: { project_name: 'yieldsense_backend' },
    promClient: { collectDefaultMetrics: {} }
});
app.use(metricsMiddleware);

// Custom Security Alerts Metric
export const securityAlerts = new Counter({
    name: 'security_alerts_total',
    help: 'Total count of security violations (auth failure, rate limit reached, etc)',
    labelNames: ['type', 'wallet']
});

export const failedTransactions = new Counter({
    name: 'transaction_failures_total',
    help: 'Total count of transaction failures',
    labelNames: ['reason']
});
 ...
 */
app.get("/api/positions/:wallet", authenticate, async (req, res) => {
    try {
        const { wallet } = req.params;
        const { sdk } = req.query;

        // Security: Ensure requested wallet matches authenticated wallet
        if ((req as any).wallet && (req as any).wallet !== wallet) {
            return res.status(403).json({ error: "Access denied: wallet mismatch" });
        }
 ...
 */
app.post("/api/position/create-or-deposit", authenticate, sensitiveActionLimiter, async (req, res) => {
    try {
        const result = await createOrDeposit(req.body);
        res.json(result);
    } catch (error: any) {
        failedTransactions.inc({ reason: 'create_deposit' });
        res.status(500).json({ error: error.message });
    }
});
 ...
 */
app.post("/api/position/withdraw", authenticate, sensitiveActionLimiter, async (req, res) => {
    try {
 ...
 * Returns an unsigned transaction
 */
app.post("/api/position/close", authenticate, sensitiveActionLimiter, async (req, res) => {
    try {
